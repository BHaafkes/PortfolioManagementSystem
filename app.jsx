        ...
        <div className="p-8"> 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6"> 
                ...
                <Card title="Change in Quantity"> 
                    {["Triodos Buy", "Triodos Sell", "Triodos Sell for Expenses", "Triodos Dividend Re-investing", "Triodos Aansluitkosten"].map(text => <ActionButton key={text} disabled>{text}</ActionButton>)} 
                    <ActionButton onClick={() => setPage('GiroBuy')}>Giro Buy</ActionButton> 
                    {["Giro Sell", "Giro iDeal"].map(text => <ActionButton key={text} disabled>{text}</ActionButton>)} 
                </Card> 
...
const GiroBuyPage = () => { 
    return ( 
        <div className="p-8 h-full flex flex-col"> 
            <h1 className="text-2xl font-bold text-gray-800 mb-6">Giro Buy</h1> 
            <div className="flex-grow bg-white border rounded-lg p-8 flex items-center justify-center"> 
                <p className="text-gray-500">Giro Buy interface will be built here.</p> 
            </div> 
        </div> 
    ); 
} 
...
export default function App() { 
    ...
    const renderPage = () => { 
        switch (page) { 
            ...
            case 'GiroBuy': 
                return <GiroBuyPage portfolioData={portfolioData} setPortfolioData={setPortfolioData} />; 
            default: 
                return <HomePage setPage={setPage} />; 
        } 
    }; 
